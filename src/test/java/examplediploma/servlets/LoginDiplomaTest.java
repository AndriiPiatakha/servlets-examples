...
public class LoginDiplomaTest {
	...
	public void shouldRedirectToLoginPageWithErrorWhenNoSuchUser() throws ServletException, IOException {
		...
		verify(rdMock).forward(req, resp);
	}
	
	@Test
	public void shouldRedirectCustomerRoleUser() throws ServletException, IOException {
		// GIVEN
		UserData user = new UserData(1, "William", "Smith", "w@email.com");
		user.setPassword("asd");
		when(userDao.getUserByEmail(anyString())).thenReturn(
				user);
		...
		verify(mockSession).setAttribute("loggedInUser", user);
	}
